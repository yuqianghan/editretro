...
Batch = namedtuple('Batch',
                   'ids src_tokens src_lengths tgt_tokens decoder_input')
...
spe_vob = codecs.open('../SPE_ChEMBL.txt') # https://github.com/XinhaoLi74/SmilesPE
...
def make_batches(lines,
    ...
    if args.inference_with_augmentation:
        ...
        lines = aug_line
        tokens = [
            task.source_dictionary.encode_line(encode_fn(
                src_str.split(separator)[0]),
    ...
    if args.has_target:
        target_tokens = [
            task.target_dictionary.encode_line(encode_fn(
                src_str.split(separator)[-1]),
                                               add_if_not_exist=False).long()
            for src_str in lines
        ]
