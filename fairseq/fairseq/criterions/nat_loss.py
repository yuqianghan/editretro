...
class LabelSmoothedDualImitationCriterion(FairseqCriterion):
    ...
    def _compute_soft_alignment_loss(self,
                                     outputs,
                                     targets,
                                     masks=None,
                                     label_smoothing=0.0,
        ...
        if masks is not None and not masks.any():
            ...
            if targets.dim() == 1:
                losses = F.nll_loss(aggr_logits,
                                    targets.to(aggr_logits.device),
                                    reduction='none')

            else:  # soft-labels
                losses = F.kl_div(aggr_logits,
                                  targets.to(aggr_logits.device),
                                  reduction='none')
                losses = losses.sum(-1)
            ...
            nll_loss = mean_ds(losses)
            if label_smoothing > 0:
                loss = nll_loss * (1 - label_smoothing
                                   ) - mean_ds(aggr_logits) * label_smoothing
            else:
                loss = nll_loss
    ...
    def _compute_BoN_loss(self,
        ...
        if masks is not None and not masks.any():
            ...
            if masks is not None:
                ...
                one_hot = torch.eye(max_len,
                                    device=probs.device).unsqueeze(0).expand(
                                        targets.size(0), -1, -1)
                curr_probs.masked_scatter_(~prob_masks, one_hot)

            next_probs = torch.cat(
                (curr_probs[:, 1:, :], torch.zeros_like(curr_probs[:,
                                                                   0:1, :])),
                dim=1)
            next_probs = torch.cat(
                (next_probs[:, :, 1:], torch.zeros_like(curr_probs[:, :,
                                                                   0:1])),
                dim=2)
            ngram_match = torch.sum(curr_probs * next_probs,
                                    dim=1).clamp(min=0, max=1)
    ...
    def forward(self, model, sample, reduce=True):
        ...
        outputs = model(src_tokens, src_lengths, lp_src_tokens,
