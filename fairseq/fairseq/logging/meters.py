...
class TimeMeter(Meter):
    ...
    def update(self, val=1):
        if isinstance(self.n, torch.Tensor) and isinstance(val, torch.Tensor):
            self.n = self.n.to(val.device)
        self.n += val
        self.i += 1
...
class MetersDict(OrderedDict):
    ...
    def state_dict(self):
        return [
            (pri, key, self[key].__class__.__name__, self[key].state_dict()) for pri, _, key in self.priorities
